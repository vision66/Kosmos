...
enum UIProgressCircleMode {
    ...
    /// 用于计时: 100%的进度, 将在指定时间内(duration)均匀的跑完
    case timer
    
    /// 用于下载: 进度(progress)的更新不均匀, 甚至可能倒退
    case counter
}
...
class UIProgressCircle: UIButton, CAAnimationDelegate {
    ...
    init(frame: CGRect, mode: UIProgressCircleMode) {
        ...
        thumbLayer.lineWidth     = trackLineWidth
    ...
    override func layoutSubviews() {
        ...
        let yCenter = bounds.size.height / 2
    ...
    var duration : CFTimeInterval { // for timer
